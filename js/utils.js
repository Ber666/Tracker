...
const Utils = {
  ...
  // Migrate old entry schema (tasks[]) to new (planned[], log[])
  // Returns true if migration occurred, false if already new format or no-op.
  migrateEntrySchema(entry) {
    if (!entry || !entry.tasks || entry.planned !== undefined) return false;

    const oldTasks = entry.tasks;
    entry.planned = [];
    entry.log = [];
    ...
    for (const task of oldTasks) {
      if (task.planned !== false) {
        entry.planned.push({
          ...
          duration: task.expectedTime || null,
          status: task.progress === 'half-done' ? 'in-progress' : (task.progress || 'not-started'),
          notes: task.comment || '',
        ...
        entry.log.push({
          ...
          duration: task.expectedTime || null,
          plannedIds: task.linkedTaskId ? [task.linkedTaskId] : [],
          notes: task.comment || '',
          ...(task.createdAt && { createdAt: task.createdAt })
        });
      }
    ...
    // Also migrate actual[] → log[] for entries saved under old name
    if (entry.actual && !entry.log?.length) {
      entry.log = entry.actual;
    }
    delete entry.actual;
    delete entry.tasks;
    return true;
